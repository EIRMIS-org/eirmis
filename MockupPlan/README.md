...
- **No build step** — every portal is a single `index.html` that opens directly in a browser.
...
├── LandingPage/           ← public hub + auth (entry point)
...
└── Check-in Staff Portal/ ← event-day scanning surface
```
...
|---|---|---|
| **Organizer** | [`Organizer Portal/`](Organizer%20Portal/) | Event CRUD + 4-step create wizard, guest list, approval queue with audited capacity override, reminders, invitation design validation, live attendance, waitlist FIFO, check-in config, staff assignments, email/audit logs |
| **Admin** | [`Admin Portal/`](Admin%20Portal/) | Organizer verification queue (`pending → active`), org-wide settings, zero-knowledge access boundary |
| **Guest** | [`Guest Portal/`](Guest%20Portal/) | Mobile-first RSVP, roster submission with `max_party_size` ceiling, rejected-attendee resubmit, headcount requests, wallet-style QR pass |
| **Check-in Staff** | [`Check-in Staff Portal/`](Check-in%20Staff%20Portal/) | Scoped scanning screen, scan-result state machine, manual search, walk-in registration |

## 5. How to run it

**Option A — open directly (no setup):**
1. Open [`LandingPage/index.html`](LandingPage/index.html) in a browser.
...
# then open http://localhost:3000/LandingPage/index.html
```
...
| [`LandingPage/README.md`](LandingPage/README.md) | Public hub & auth mockup |
| [`Organizer Portal/README.md`](Organizer%20Portal/README.md) | Organizer workspace mockup |
| [`Admin Portal/README.md`](Admin%20Portal/README.md) | Admin verification & settings mockup |
| [`Guest Portal/README.md`](Guest%20Portal/README.md) | Guest self-service mockup |
| [`Check-in Staff Portal/README.md`](Check-in%20Staff%20Portal/README.md) | Check-in scanning mockup |